...
async function refresh(s){
  ...
  const r=await fetch(U+'/auth/v1/token?grant_type=refresh_token',{
    ...
    body:JSON.stringify({refresh_token:s.refresh_token}),cache:'no-store'
  });
  if(!r.ok)return s;
  const n=await r.json();store(n);return n;
}
...
function setValue(id,v){const e=document.querySelector(id);if(e)e.value=v??''}
function applyProfile(p,s){
  if(!p)return;
  const n=p.full_name||p.email||s?.user?.email||'J&G Member';
  setValue('#mdName',p.full_name);setValue('#mdCompany',p.company);setValue('#mdPhone',p.phone);
  setValue('#mdEmail',p.email||s?.user?.email);setValue('#mdStreet',p.street);setValue('#mdPostal',p.postal_code);
  setValue('#mdCity',p.city);setValue('#mdCountry',p.country||'Schweiz');
  const name=document.querySelector('#userName');if(name)name.textContent=n;
  const hello=document.querySelector('#hello');if(hello)hello.textContent='Guten Abend, '+n;
  const avatar=document.querySelector('#avatar');if(avatar)avatar.textContent=n.split(/\s+/).filter(Boolean).map(x=>x[0]).join('').slice(0,2).toUpperCase();
}

async function saveProfile(btn){
  const m=messageEl(btn);m.textContent='Daten werden gespeichert …';
  let s=session();
  if(!s?.access_token||!s?.user?.id){m.textContent='Sitzung abgelaufen. Bitte erneut anmelden.';return}
  const body={
    full_name:document.querySelector('#mdName')?.value.trim()||'',
    company:document.querySelector('#mdCompany')?.value.trim()||'',
    phone:document.querySelector('#mdPhone')?.value.trim()||'',
    street:document.querySelector('#mdStreet')?.value.trim()||'',
    postal_code:document.querySelector('#mdPostal')?.value.trim()||'',
    city:document.querySelector('#mdCity')?.value.trim()||'',
    country:document.querySelector('#mdCountry')?.value.trim()||'Schweiz'
  };
  const doPatch=async()=>fetch(U+'/rest/v1/profiles?id=eq.'+encodeURIComponent(s.user.id)+'&select=*',{
  ...
  try{
    let r=await doPatch();
    if(r.status===401){s=await refresh(s);r=await doPatch()}
    if(!r.ok)throw new Error((await r.text())||('HTTP '+r.status));
    const rows=await r.json();const p=rows?.[0]||body;
    applyProfile(p,s);
    m.textContent='✓ Ihre Daten wurden gespeichert.';
  }catch(e){m.textContent='Speichern nicht möglich: '+e.message}
